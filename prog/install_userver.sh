...
function do_install()
{
	...
	# @sheng 2009-06-26 force
	cp -rf $ARG_src/lib*/* $ARG_dst/lib/ || return 1
...
function do_upgrade()
{
	...
	service crond restart
        ...
        [ ! -f $ARG_src/lib/libmp4av.so.0 ] || ln -s $ARG_dst/lib/libmp4av.so $ARG_dst/lib/libmp4av.so.0 || return 1
        [ ! -f $ARG_src/lib/libmp4util.so.0 ] || ln -s $ARG_dst/lib/libmp4util.so $ARG_dst/lib/libmp4util.so.0 || return 1
        [ ! -f $ARG_src/lib/libmp4v2.so.0 ] || ln -s $ARG_dst/lib/libmp4v2.so $ARG_dst/lib/libmp4v2.so.0 || return 1
        [ ! -f $ARG_src/lib/libsdp.so.0 ] || ln -s $ARG_dst/lib/libsdp.so $ARG_dst/lib/libsdp.so.0 || return 1
        safe_cp $ARG_src/install.sh $ARG_dst/install.sh || return 1
        return 0
}
