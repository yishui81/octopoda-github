...
struct HttpVCTable
{
  ...
  HttpVCTableEntry *find_entry(SockStream *);
...
class HttpStateMachine: public UTaskObj
{
...
public:
	...
	static void _instantiate_func(HttpStateMachine * prototype, HttpStateMachine * new_instance);
	static void _make_scatter_list(HttpStateMachine * prototype);
	...
	Connector* do_transform_open();
	Connector* do_post_transform_open();
...
protected:
  ...
  HttpVCTableEntry *user_agent_entry;
...
//Y! ebalsa: remap handlers
  ...
  void setup_cache_write_transfer(HttpCacheSM * c_sm,
                                  Connector * source_vc, HTTPInfo * store_info, int64_t skip_bytes, const char *name);
  ...
  int (HttpStateMachine::*m_last_state) (int event, void *data);
...
public:
  LINK(HttpStateMachine, debug_link);
