...
//  state machines
DLL<HttpStateMachine> debug_sm_list;
 ...
 */
static double
milestone_difference(const int64_t start, const int64_t end)
{
  if (end == 0) {
    return -1;
  }
...
static double
milestone_difference_msec(const int64_t start, const int64_t end)
{
  if (end == 0) {
    return -1;
  }
...
void
HttpStateMachine::_make_scatter_list(HttpStateMachine * prototype)
{
	int j;
	int total_len = sizeof(HttpStateMachine);
...
inline HttpCacheSM &
HttpStateMachine::get_cache_sm()
{
	return cache_sm;
}

//Function to get the user_agent_entry pointer - YTS Team, yamsat
inline HttpVCTableEntry *
HttpStateMachine::get_user_agent_entry()
{
	return user_agent_entry;
}

SparseClassAllocator<HttpStateMachine> httpSMAllocator("httpSMAllocator", 128, 16, HttpStateMachine::_instantiate_func);


inline HttpStateMachine *
HttpStateMachine::allocate()
{
	  extern SparseClassAllocator<HttpStateMachine> httpSMAllocator;
	  return httpSMAllocator.alloc();
}

inline void
HttpStateMachine::remove_user_agent_entry()
{
	vc_table.remove_entry(user_agent_entry);
	user_agent_entry = NULL;
}

inline void
HttpStateMachine::remove_server_entry()
{
	if (server_entry) {
...
inline int
HttpStateMachine::write_response_header_into_buffer(HTTPHdr * h, MIOBuffer * b)
{
...
inline void
HttpStateMachine::add_history_entry(const char *fileline, int event, int reentrant)
{
...
inline int
HttpStateMachine::find_server_buffer_size()
{
...
//
inline APIHook *
HttpStateMachine::txn_hook_get(TSHttpHookID id)
{
  return api_hooks.get(id);
}

inline void
HttpStateMachine::add_cache_sm()
{
...
inline bool
HttpStateMachine::is_transparent_passthrough_allowed()
{
...
void
HttpStateMachine::_instantiate_func(HttpStateMachine * prototype, HttpStateMachine * new_instance)
{
...
int
handle_open  (const URE_Msg& msg){
	return 0;
}

int
handle_close (UWorkEnv * orign_uwe, long retcode){
	return 0;
}

//timer
int
handle_timeout( const TimeValue & origts, long time_id, const void * act ){
	return 0;
}

//message
int
handle_message( const URE_Msg & msg )
{
	this.msg = msg;
	//FIXME function return value processing
	do_api_call_internal();
	return 0;
...
bool
HttpStateMachine::decide_cached_url(URL * s_url)
{
...
void
HttpStateMachine::set_ua_half_close_flag()
{
...
int
HttpStateMachine::state_add_to_list(int event, void * /* data ATS_UNUSED */)
{
  ...
  if (t_state.http_config_param->enable_http_info) {
    STATE_ENTER(&HttpStateMachine::state_add_to_list, event);
...
int
HttpStateMachine::state_remove_from_list(int event, void * /* data ATS_UNUSED */)
{
  ...
  //   state page list
  if (t_state.http_config_param->enable_http_info) {
    STATE_ENTER(&HttpStateMachine::state_remove_from_list, event);
    ink_assert(event == EVENT_NONE || event == EVENT_INTERVAL);
    ...
    MUTEX_TRY_LOCK(lock, HttpSMList[bucket].mutex, mutex->thread_holding);
    if (!lock) {
      HTTP_SM_SET_DEFAULT_HANDLER(&HttpStateMachine::state_remove_from_list);
...
int
HttpStateMachine::kill_this_async_hook(int /* event ATS_UNUSED */, void * /* data ATS_UNUSED */)
{
...
void
HttpStateMachine::attach_client_session(HttpClientSession * client_session, IOBufferReader * buffer_reader)
{

	milestones.user_agent_begin = ink_get_hrtime();
	ink_assert(client_session != NULL);
...
//	mutex = client_vc->mutex;
	...
	//   vc table
	user_agent_entry = vc_table.new_entry();
	user_agent_entry->vc = client_session;
	user_agent_entry->vc_type = HTTP_UA_VC;

	Connector* netvc = client_session->get_connector();
	...
	user_agent_entry->vc_handler = &HttpStateMachine::state_read_client_request_header;
	...
	user_agent_entry->read_vio = client_vc->do_io_read(this, 0, buffer_reader->mbuf);
...
{
  ink_assert(user_agent_entry->vc_handler == &HttpStateMachine::state_read_client_request_header);

  user_agent_entry->read_vio = ua_session->do_io_read(this, INT64_MAX, ua_buffer_reader->mbuf);
  // The header may already be in the buffer if this
  //  a request from a keep-alive connection
  if (ua_buffer_reader->read_avail() > 0)
    handleEvent(VC_EVENT_READ_READY, user_agent_entry->read_vio);
}
...
int
HttpStateMachine::state_read_client_request_header(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_read_client_request_header, event);

  ink_assert(user_agent_entry->read_vio == (VIO *) data);
  ink_assert(server_entry == NULL);
  ink_assert(server_session == NULL);

  int bytes_used = 0;
  ink_assert(user_agent_entry->eos == false);
  ...
  case VC_EVENT_ACTIVE_TIMEOUT:
    ...
    vc_table.cleanup_entry(user_agent_entry);
    user_agent_entry = NULL;
  ...
  if (state != PARSE_CONT || user_agent_entry->eos ||
    ...
    http_parser_clear(&http_parser);
    user_agent_entry->vc_handler = &HttpStateMachine::state_watch_for_client_abort;
  ...
  case PARSE_ERROR:
    ...
    user_agent_entry->read_vio->nbytes = user_agent_entry->read_vio->ndone;

    call_transact_and_set_next_state(HttpTransact::BadRequest);
    break;

  case PARSE_CONT:
    if (user_agent_entry->eos) {
	...
	break;
      ...
      user_agent_entry->read_vio->reenable();
...
#ifdef PROXY_DRAIN
int
HttpStateMachine::state_drain_client_request_body(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_drain_client_request_body, event);

  ink_assert(user_agent_entry->read_vio == (VIO *) data);
  ink_assert(user_agent_entry->vc == ua_session);
  ...
  case VC_EVENT_READ_READY:
    {
      ...
      user_agent_entry->read_vio->reenable_re();
      break;
    }
  case VC_EVENT_READ_COMPLETE:
    {
      ...
      user_agent_entry->vc_handler = &HttpStateMachine::state_watch_for_client_abort;
      user_agent_entry->read_vio = user_agent_entry->vc->do_io_read(this, INT64_MAX, ua_buffer_reader->mbuf);
...
int
HttpStateMachine::state_watch_for_client_abort(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_watch_for_client_abort, event);
...
void
HttpStateMachine::setup_push_read_response_header()
{
  ink_assert(server_session == NULL);
  ink_assert(server_entry == NULL);
  ...
  user_agent_entry->vc_handler = &HttpStateMachine::state_read_push_response_header;
  ...
  //  request header
  ink_assert(user_agent_entry->read_vio);
  ...
  if (ua_buffer_reader->read_avail() > 0) {
    if (user_agent_entry->eos) {
      resp_hdr_state = state_read_push_response_header(VC_EVENT_EOS, user_agent_entry->read_vio);
    } else {
      resp_hdr_state = state_read_push_response_header(VC_EVENT_READ_READY, user_agent_entry->read_vio);
    }
  }
...
int
HttpStateMachine::state_read_push_response_header(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_read_push_response_header, event);
  ink_assert(user_agent_entry->read_vio == (VIO *) data);
...
//////////////////////////////////////////////////////////////////////////////
int
HttpStateMachine::state_raw_http_server_open(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_raw_http_server_open, event);
  ink_assert(server_entry == NULL);
...
//
int
HttpStateMachine::state_request_wait_for_transform_read(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_request_wait_for_transform_read, event);
  ...
  case TRANSFORM_READ_READY:
    ...
    // FALLTHROUGH
  default:
    state_common_wait_for_transform_read(&post_transform_info, &HttpStateMachine::tunnel_handler_post, event, data);
    break;
  }
...
//
int
HttpStateMachine::state_response_wait_for_transform_read(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_response_wait_for_transform_read, event);
  int64_t size = *((int64_t*)data);
  ...
  case TRANSFORM_READ_READY:
    ...
    break;
  default:
    state_common_wait_for_transform_read(&transform_info, &HttpStateMachine::tunnel_handler, event, data);
    break;
  }
...
//
int
HttpStateMachine::state_common_wait_for_transform_read(HttpTransformInfo * t_info, HttpSMHandler tunnel_handler, int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_common_wait_for_transform_read, event);
...
//
int
HttpStateMachine::state_api_callback(int event, void *data)
{
  ...
  STATE_ENTER(&HttpStateMachine::state_api_callback, event);
...
int
HttpStateMachine::state_api_callout(int event, void *data)
{
  ...
  if (event != EVENT_NONE) {
    STATE_ENTER(&HttpStateMachine::state_api_callout, event);
  }
    ...
    if ((cur_hook_id >= 0) && (cur_hook_id < TS_HTTP_LAST_HOOK)) {
      ...
      if (cur_hook) {
        ...
        if (cur_hook->m_cont->mutex) {
          ...
          if (!plugin_lock) {
            HTTP_SM_SET_DEFAULT_HANDLER(&HttpStateMachine::state_api_callout);
...
//
void
HttpStateMachine::handle_api_return()
{
...
//////////////////////////////////////////////////////////////////////////////
int
HttpStateMachine::state_http_server_open(int event, void *data)
{
  DebugSM("http_track", "entered inside state_http_server_open");
  STATE_ENTER(&HttpStateMachine::state_http_server_open, event);
...
int
HttpStateMachine::state_read_server_response_header(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_read_server_response_header, event);
...
int
HttpStateMachine::state_send_server_request_header(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_send_server_request_header, event);
  ink_assert(server_entry != NULL);
...
void
HttpStateMachine::process_srv_info(HostDBInfo * r)
{
...
void
HttpStateMachine::process_hostdb_info(HostDBInfo * r)
{
  if (r && !r->failed()) {
...
//////////////////////////////////////////////////////////////////////////////
int
HttpStateMachine::state_hostdb_lookup(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_hostdb_lookup, event);
...
//    ink_assert (m_origin_server_vc == 0);
  ...
  ink_assert(t_state.req_flavor == HttpTransact::REQ_FLAVOR_SCHEDULED_UPDATE ||
             t_state.req_flavor == HttpTransact::REQ_FLAVOR_REVPROXY || user_agent_entry->vc != NULL);
  ...
  case EVENT_HOST_DB_LOOKUP:
    ...
    case EVENT_SRV_LOOKUP:
    {
      ...
      Action *dns_lookup_action_handle = hostDBProcessor.getbyname_imm(this,
                                                                 (process_hostdb_info_pfn) & HttpStateMachine::
                                                                 process_hostdb_info,
                                                                 host_name, 0,
                                                                 opt);
...
int
HttpStateMachine::state_hostdb_reverse_lookup(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_hostdb_reverse_lookup, event);
...
//////////////////////////////////////////////////////////////////////////////
int
HttpStateMachine::state_mark_os_down(int event, void *data)
{
...
//////////////////////////////////////////////////////////////////////////
int
HttpStateMachine::state_handle_stat_page(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_handle_stat_page, event);
  switch (event) {
...
//{
 // STATE_ENTER(&HttpSM::state_auth_lookup, event);
  ...
  //ink_release_assert(user_agent_entry != NULL);
  //pending_action = NULL;
...
///////////////////////////////////////////////////////////////
int
HttpStateMachine::state_icp_lookup(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_icp_lookup, event);

  // user_agent_entry is NULL for scheduled updates
  ink_release_assert(user_agent_entry != NULL ||
...
//////////////////////////////////////////////////////////////////////////
int
HttpStateMachine::state_cache_open_write(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine:state_cache_open_write, event);
...
inline void
HttpStateMachine::setup_cache_lookup_complete_api()
{
...
//////////////////////////////////////////////////////////////////////////
int
HttpStateMachine::state_cache_open_read(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_cache_open_read, event);
...
//
void
HttpStateMachine::tunnel_handler_post_or_put(HttpTunnelProducer * p)
{
	...
	case HTTP_SM_POST_SUCCESS:
		...
		ink_release_assert(user_agent_entry->in_tunnel == false);
...
//
int
HttpStateMachine::tunnel_handler_post(int event, void *data)
{
	STATE_ENTER(&HttpStateMachine::tunnel_handler_post, event);
...
int
HttpStateMachine::tunnel_handler_cache_fill(int event, void *data)
{
	STATE_ENTER(&HttpStateMachine::tunnel_handler_cache_fill, event);
...
int
HttpStateMachine::tunnel_handler_100_continue(int event, void *data)
{
	STATE_ENTER(&HttpStateMachine::tunnel_handler_100_continue, event);
...
int
HttpStateMachine::tunnel_handler_push(int event, void *data)
{
	STATE_ENTER(&HttpStateMachine::tunnel_handler_push, event);
...
int
HttpStateMachine::tunnel_handler(int event, void *data)
{
	STATE_ENTER(&HttpStateMachine::tunnel_handler, event);
...
bool
HttpStateMachine::is_http_server_eos_truncation(HttpTunnelProducer * p)
{
...
int
HttpStateMachine::tunnel_handler_server(int event, HttpTunnelProducer * p)
{
	STATE_ENTER(&HttpStateMachine::tunnel_handler_server, event);
	...
	if (close_connection) {
		...
		*/
		if (ua_session && ua_session->f_outbound_transparent &&
...
//
int
HttpStateMachine::tunnel_handler_100_continue_ua(int event, HttpTunnelConsumer * c)
{
	STATE_ENTER(&HttpStateMachine::tunnel_handler_100_continue_ua, event);
	...
	case VC_EVENT_WRITE_COMPLETE:
		...
		user_agent_entry->in_tunnel = false;
...
bool
HttpStateMachine::is_bg_fill_necessary(HttpTunnelConsumer * c)
{
	...
	if (((c->producer->num_consumers > 1 &&
		c->producer->vc_type == HT_HTTP_SERVER) ||
		(c->producer->num_consumers > 1 &&
...
int
HttpStateMachine::tunnel_handler_ua(int event, HttpTunnelConsumer * c)
{
  bool close_connection = true;
  HttpTunnelProducer *p = NULL;
  HttpTunnelConsumer *selfc = NULL;

  STATE_ENTER(&HttpStateMachine::tunnel_handler_ua, event);
  ink_assert(c->vc == ua_session);
  milestones.ua_close = ink_get_hrtime();
  ...
  ink_assert(user_agent_entry->vc == c->vc);
  if (close_connection) {
...
int
HttpStateMachine::tunnel_handler_ua_push(int event, HttpTunnelProducer * p)
{
  STATE_ENTER(&HttpStateMachine::tunnel_handler_ua_push, event);
...
int
HttpStateMachine::tunnel_handler_cache_read(int event, HttpTunnelProducer * p)
{
  STATE_ENTER(&HttpStateMachine::tunnel_handler_cache_read, event);
...
int
HttpStateMachine::tunnel_handler_cache_write(int event, HttpTunnelConsumer * c)
{
  STATE_ENTER(&HttpStateMachine::tunnel_handler_cache_write, event);
...
int
HttpStateMachine::tunnel_handler_post_ua(int event, HttpTunnelProducer * p)
{
  STATE_ENTER(&HttpStateMachine::tunnel_handler_post_ua, event);
  ...
  case VC_EVENT_ACTIVE_TIMEOUT:
    ...
    hsm_release_assert(user_agent_entry->in_tunnel == true);
  ...
  case HTTP_TUNNEL_EVENT_PRECOMPLETE:
    ...
    //   timeouts
    user_agent_entry->vc_handler = &HttpStateMachine::state_watch_for_client_abort;
    user_agent_entry->read_vio = p->vc->do_io_read(this, INT64_MAX, ua_buffer_reader->mbuf);
    break;
  default:
    ink_release_assert(0);
  }
...
int
HttpStateMachine::tunnel_handler_for_partial_post(int event, void * /* data ATS_UNUSED */)
{
  STATE_ENTER(&HttpStateMachine::tunnel_handler_for_partial_post, event);
  tunnel.deallocate_buffers();
  tunnel.reset();
...
int
HttpStateMachine::tunnel_handler_post_server(int event, HttpTunnelConsumer * c)
{
  STATE_ENTER(&HttpStateMachine::tunnel_handler_post_server, event);
  ...
  case VC_EVENT_ACTIVE_TIMEOUT:
    ...
    ink_assert(ua_producer->vc == user_agent_entry->vc);
    ...
    if (enable_redirection) {
      if (ua_producer->vc_type == HT_STATIC && event != VC_EVENT_ERROR && event != VC_EVENT_EOS) {
        user_agent_entry->read_vio = ua_producer->vc->do_io_read(this, INT64_MAX, c->producer->read_buffer);
...
int
HttpStateMachine::tunnel_handler_ssl_producer(int event, HttpTunnelProducer * p)
{

  STATE_ENTER(&HttpStateMachine::tunnel_handler_ssl_producer, event);
...
int
HttpStateMachine::tunnel_handler_ssl_consumer(int event, HttpTunnelConsumer * c)
{
  STATE_ENTER(&HttpStateMachine::tunnel_handler_ssl_consumer, event);
...
int
HttpStateMachine::tunnel_handler_transform_write(int event, HttpTunnelConsumer * c)
{
  STATE_ENTER(&HttpStateMachine::tunnel_handler_transform_write, event);
...
int
HttpStateMachine::tunnel_handler_transform_read(int event, HttpTunnelProducer * p)
{
  STATE_ENTER(&HttpStateMachine::tunnel_handler_transform_read, event);
...
int
HttpStateMachine::tunnel_handler_plugin_agent(int event, HttpTunnelConsumer * c)
{
  STATE_ENTER(&HttpStateMachine::tunnel_handler_plugin_client, event);
...
int
HttpStateMachine::state_srv_lookup(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_srv_lookup, event);
...
int
HttpStateMachine::state_remap_request(int event, void * /* data ATS_UNUSED */)
{
  STATE_ENTER(&HttpStateMachine::state_remap_request, event);
...
void
HttpStateMachine::do_remap_request(bool run_inline)
{
...
void
HttpStateMachine::do_hostdb_lookup()
{
/*
    //////////////////////////////////////////
  ...
  if (use_srv_records) {
    ...
    Action *srv_lookup_action_handle =
      hostDBProcessor.getSRVbyname_imm(this, (process_srv_info_pfn) & HttpStateMachine::process_srv_info, d, 0, opt);
    ...
    Action *dns_lookup_action_handle = hostDBProcessor.getbyname_imm(this, (process_hostdb_info_pfn) & HttpStateMachine::process_hostdb_info, t_state.dns_info.lookup_name, 0, opt);
...
void
HttpStateMachine::do_hostdb_reverse_lookup()
{
  ink_assert(t_state.dns_info.lookup_name != NULL);
  ink_assert(pending_action == NULL);
 ...
 */
void
HttpStateMachine::parse_range_and_compare(MIMEField *field, int64_t content_length)
{
...
void
HttpStateMachine::calculate_output_cl(int64_t content_length, int64_t num_chars)
{
  int i;
...
void
HttpStateMachine::do_range_parse(MIMEField *range_field)
{
...
// HttpTunnel
void
HttpStateMachine::do_range_setup_if_necessary()
{
  MIMEField *field;
...
void
HttpStateMachine::do_cache_lookup_and_read()
{
  // TODO decide whether to uncomment after finish testing redirect
  //ink_assert(server_session == NULL);
...
void
HttpStateMachine::do_cache_delete_all_alts(Continuation * cont)
{
...
inline void
HttpStateMachine::do_cache_prepare_write()
{
...
inline void
HttpStateMachine::do_cache_prepare_write_transform()
{
...
void
HttpStateMachine::do_cache_prepare_update()
{
  if (t_state.cache_info.object_read != NULL &&
...
void
HttpStateMachine::do_cache_prepare_action(HttpCacheSM * c_sm, CacheHTTPInfo * object_read_info, bool retry, bool allow_multiple)
{
...
//////////////////////////////////////////////////////////////////////////
void
HttpStateMachine::do_http_server_open(bool raw)
{
	...
	// user_agent_entry can be null if a scheduled update is also a reverse proxy
	...
	ink_assert(user_agent_entry != NULL ||
			 t_state.req_flavor == HttpTransact::REQ_FLAVOR_SCHEDULED_UPDATE ||
			 t_state.req_flavor == HttpTransact::REQ_FLAVOR_REVPROXY);
	...
	if (t_state.pCongestionEntry != NULL) {
		...
		if (t_state.pCongestionEntry->F_congested() &&
				(!t_state.pCongestionEntry->proxy_retry(milestones.server_connect))) {
	...
	if (raw == false &&
			t_state.txn_conf->share_server_sessions &&
		...
		if (t_state.method != HTTP_WKSIDX_CONNECT) {
		  ...
		  connect_action_handle = netProcessor.connect_re(this,     // state machine
			  ...
			  if (t_state.method == HTTP_WKSIDX_POST ||
					  t_state.method == HTTP_WKSIDX_PUT) {
...
void
HttpStateMachine::do_icp_lookup()
{
  ink_assert(pending_action == NULL);
...
void
HttpStateMachine::do_api_callout_internal()
{
	...
	case HttpTransact::HTTP_API_SM_SHUTDOWN:
		...
		if (callout_state == HTTP_API_IN_CALLOUT ||
...
VConnection *
HttpStateMachine::do_post_transform_open()
{
...
VConnection *
HttpStateMachine::do_transform_open()
{
...
void
HttpStateMachine::mark_host_failure(HostDBInfo * info, time_t time_down)
{
  char addrbuf[INET6_ADDRPORTSTRLEN];
...
void
HttpStateMachine::set_ua_abort(HttpTransact::AbortState_t ua_abort, int event)
{
...
void
HttpStateMachine::mark_server_down_on_client_abort()
{
  /////////////////////////////////////////////////////
...
//
void
HttpStateMachine::release_server_session(bool serve_from_cache)
{
...
void
HttpStateMachine::handle_post_failure()
{
  STATE_ENTER(&HttpStateMachine::handle_post_failure, VC_EVENT_NONE);
  ...
  if (false == t_state.redirect_info.redirect_in_process) {
    user_agent_entry->read_vio = ua_session->do_io_read(this, 0, NULL);
  }
  user_agent_entry->in_tunnel = false;
  server_entry->in_tunnel = false;
  tunnel.deallocate_buffers();
...
//
void
HttpStateMachine::handle_http_server_open()
{
...
//
void
HttpStateMachine::handle_server_setup_error(int event, void *data)
{
  VIO *vio = (VIO *) data;
  ink_assert(vio != NULL);
  ...
  if (tunnel.is_tunnel_active()) {
    ...
    if (c == NULL && post_transform_info.vc) {
      ...
      ink_assert(user_agent_entry->vc == ua_producer->vc);
  ...
  STATE_ENTER(&HttpStateMachine::handle_server_setup_error, callout_state);
...
void
HttpStateMachine::setup_transform_to_server_transfer()
{
  ink_assert(post_transform_info.vc != NULL);
  ...
  HTTP_SM_SET_DEFAULT_HANDLER(&HttpStateMachine::tunnel_handler_post);
  ...
  tunnel.add_consumer(server_entry->vc,
                      post_transform_info.vc, &HttpStateMachine::tunnel_handler_post_server, HT_HTTP_SERVER, "http server post");
  server_entry->in_tunnel = true;
...
#ifdef PROXY_DRAIN
void
HttpStateMachine::do_drain_request_body()
{
  ...
  if (client_request_body_bytes < post_bytes) {
    ...
    user_agent_entry->vc_handler = &HttpStateMachine::state_drain_client_request_body;
    user_agent_entry->read_vio = user_agent_entry->vc->do_io_read(this, post_bytes - client_request_body_bytes, ua_buffer_reader->mbuf);
  } else {
    call_transact_and_set_next_state(NULL);
  }
}
...
void
HttpStateMachine::do_setup_post_tunnel(HttpVC_t to_vc_type)
{
  ...
  if (t_state.redirect_info.redirect_in_process && enable_redirection &&
    ...
    p = tunnel.add_producer(user_agent_entry->vc, post_bytes - transfered_bytes, buf_start,
                            &HttpStateMachine::tunnel_handler_post_ua, HT_HTTP_CLIENT, "user agent post");
  }
  user_agent_entry->in_tunnel = true;

  switch (to_vc_type) {
  case HTTP_TRANSFORM_VC:
    HTTP_SM_SET_DEFAULT_HANDLER(&HttpStateMachine::state_request_wait_for_transform_read);
    ink_assert(post_transform_info.entry != NULL);
    ink_assert(post_transform_info.entry->vc == post_transform_info.vc);
    tunnel.add_consumer(post_transform_info.entry->vc,
                        user_agent_entry->vc, &HttpStateMachine::tunnel_handler_transform_write, HT_TRANSFORM, "post transform");
    post_transform_info.entry->in_tunnel = true;
    break;
  case HTTP_SERVER_VC:
    //YTS Team, yamsat Plugin
    ...
    if (post_redirect) {
      HTTP_SM_SET_DEFAULT_HANDLER(&HttpStateMachine::tunnel_handler_for_partial_post);
      tunnel.add_consumer(server_entry->vc,
                          HTTP_TUNNEL_STATIC_PRODUCER,
                          &HttpStateMachine::tunnel_handler_post_server, HT_HTTP_SERVER, "redirect http server post");
    } else {
      HTTP_SM_SET_DEFAULT_HANDLER(&HttpStateMachine::tunnel_handler_post);
      tunnel.add_consumer(server_entry->vc,
                          user_agent_entry->vc, &HttpStateMachine::tunnel_handler_post_server, HT_HTTP_SERVER, "http server post");
    }
    server_entry->in_tunnel = true;
    break;
  default:
...
//
void
HttpStateMachine::perform_cache_write_action()
{
...
void
HttpStateMachine::issue_cache_update()
{
  ink_assert(cache_sm.cache_write_vc != NULL);
  if (cache_sm.cache_write_vc) {
...
int
HttpStateMachine::write_header_into_buffer(HTTPHdr * h, MIOBuffer * b)
{
  int bufindex;
  int dumpoffset;
  int done, tmp;
...
void
HttpStateMachine::attach_server_session(HttpServerSession * s)
{
  hsm_release_assert(server_session == NULL);
  ...
  server_entry->vc_handler = &HttpStateMachine::state_send_server_request_header;
...
void
HttpStateMachine::setup_server_send_request_api()
{
...
void
HttpStateMachine::setup_server_read_response_header()
{
  ink_assert(server_session != NULL);
  ink_assert(server_entry != NULL);
  ...
  server_entry->vc_handler = &HttpStateMachine::state_read_server_response_header;
...
void
HttpStateMachine::setup_cache_read_transfer()
{
  ...
  HttpTunnelProducer *p = tunnel.add_producer(cache_sm.cache_read_vc,
                                              doc_size, buf_start, &HttpStateMachine::tunnel_handler_cache_read, HT_CACHE_READ,
                                              "cache read");
  tunnel.add_consumer(user_agent_entry->vc, cache_sm.cache_read_vc, &HttpStateMachine::tunnel_handler_ua, HT_HTTP_CLIENT, "user agent");
...
HttpTunnelProducer *
HttpStateMachine::setup_cache_transfer_to_transform()
{
  int64_t alloc_index;
  int64_t doc_size;
  ...
  HTTP_SM_SET_DEFAULT_HANDLER(&HttpStateMachine::state_response_wait_for_transform_read);

  HttpTunnelProducer *p = tunnel.add_producer(cache_sm.cache_read_vc,
                                              doc_size,
                                              buf_start,
                                              &HttpStateMachine::tunnel_handler_cache_read,
                                              HT_CACHE_READ,
                                              "cache read");

  tunnel.add_consumer(transform_info.vc,
                      cache_sm.cache_read_vc, &HttpStateMachine::tunnel_handler_transform_write, HT_TRANSFORM, "transform write");
...
void
HttpStateMachine::setup_cache_write_transfer(HttpCacheSM * c_sm,
                                   VConnection * source_vc, HTTPInfo * store_info, int64_t skip_bytes, const char *name)
{
  ...
  tunnel.add_consumer(c_sm->cache_write_vc,
                      source_vc, &HttpStateMachine::tunnel_handler_cache_write, HT_CACHE_WRITE, name, skip_bytes);

  c_sm->cache_write_vc = NULL;
}

void
HttpStateMachine::setup_100_continue_transfer()
{
  ...
  HTTP_SM_SET_DEFAULT_HANDLER(&HttpStateMachine::tunnel_handler_100_continue);
  ...
  tunnel.add_consumer(user_agent_entry->vc,
                      HTTP_TUNNEL_STATIC_PRODUCER,
                      &HttpStateMachine::tunnel_handler_100_continue_ua, HT_HTTP_CLIENT, "user agent");
...
//////////////////////////////////////////////////////////////////////////
void
HttpStateMachine::setup_error_transfer()
{
...
void
HttpStateMachine::setup_internal_transfer(HttpSMHandler handler_arg)
{
  ...
  tunnel.add_consumer(user_agent_entry->vc,
                      HTTP_TUNNEL_STATIC_PRODUCER, &HttpStateMachine::tunnel_handler_ua, HT_HTTP_CLIENT, "user agent");
...
//
int
HttpStateMachine::find_http_resp_buffer_size(int64_t content_length)
{
  int64_t buf_size;
  int64_t alloc_index;
...
//
int64_t
HttpStateMachine::server_transfer_init(MIOBuffer * buf, int hdr_size)
{
  int64_t nbytes;
...
HttpTunnelProducer *
HttpStateMachine::setup_server_transfer_to_transform()
{
  int64_t alloc_index;
  int64_t nbytes;
  ...
  HttpTunnelProducer *p = tunnel.add_producer(server_entry->vc,
                                              nbytes,
                                              buf_start,
                                              &HttpStateMachine::tunnel_handler_server,
                                              HT_HTTP_SERVER,
                                              "http server");

  tunnel.add_consumer(transform_info.vc,
                      server_entry->vc, &HttpStateMachine::tunnel_handler_transform_write, HT_TRANSFORM, "transform write");
...
HttpStateMachine::setup_transfer_from_transform()
{
  ...
  tunnel.add_consumer(user_agent_entry->vc, transform_info.vc, &HttpStateMachine::tunnel_handler_ua, HT_HTTP_CLIENT, "user agent");
...
HttpTunnelProducer *
HttpStateMachine::setup_transfer_from_transform_to_cache_only()
{
  int64_t alloc_index = find_server_buffer_size();
  MIOBuffer *buf = new_MIOBuffer(alloc_index);
  IOBufferReader *buf_start = buf->alloc_reader();
...
void
HttpStateMachine::setup_server_transfer_to_cache_only()
{
...
void
HttpStateMachine::setup_server_transfer()
{
  int64_t alloc_index, hdr_size;
  int64_t nbytes;
  ...
  tunnel.add_consumer(user_agent_entry->vc, server_entry->vc, &HttpStateMachine::tunnel_handler_ua, HT_HTTP_CLIENT, "user agent");
...
void
HttpStateMachine::setup_push_transfer_to_cache()
{
  ...
  HTTP_SM_SET_DEFAULT_HANDLER(&HttpStateMachine::tunnel_handler_push);

  // TODO: Should we do something with the HttpTunnelProducer* returned?
  tunnel.add_producer(user_agent_entry->vc, nbytes, buf_start, &HttpStateMachine::tunnel_handler_ua_push,
                      HT_HTTP_CLIENT, "user_agent");
  setup_cache_write_transfer(&cache_sm, user_agent_entry->vc, &t_state.cache_info.object_store, 0, "cache write");

  user_agent_entry->in_tunnel = true;
}

void
HttpStateMachine::setup_blind_tunnel(bool send_response_hdr)
{
  ...
  p_os = tunnel.add_producer(server_entry->vc,
                             -1, r_to, &HttpStateMachine::tunnel_handler_ssl_producer, HT_HTTP_SERVER, "http server - tunnel");

  c_ua = tunnel.add_consumer(user_agent_entry->vc,
                             server_entry->vc,
                             &HttpStateMachine::tunnel_handler_ssl_consumer, HT_HTTP_CLIENT, "user agent - tunnel");


  p_ua = tunnel.add_producer(user_agent_entry->vc,
                             -1, r_from, &HttpStateMachine::tunnel_handler_ssl_producer, HT_HTTP_CLIENT, "user agent - tunnel");

  c_os = tunnel.add_consumer(server_entry->vc,
                             user_agent_entry->vc,
                             &HttpStateMachine::tunnel_handler_ssl_consumer, HT_HTTP_SERVER, "http server - tunnel");
...
void
HttpStateMachine::setup_plugin_agents(HttpTunnelProducer* p)
{
  ...
  while (agent) {
    ...
    tunnel.add_consumer(contp, p->vc, &HttpStateMachine::tunnel_handler_plugin_agent, HT_HTTP_CLIENT, "plugin agent");
...
inline void
HttpStateMachine::transform_cleanup(TSHttpHookID hook, HttpTransformInfo * info)
{
...
void
HttpStateMachine::plugin_agents_cleanup()
{
...
//
void
HttpStateMachine::dump_state_on_assert()
{
...
void
HttpStateMachine::dump_state_hdr(HTTPHdr *h, const char *s)
{
...
bool
HttpStateMachine::do_congestion_control_lookup()
{
  ink_assert(pending_action == NULL);
...
int
HttpStateMachine::state_congestion_control_lookup(int event, void *data)
{
  STATE_ENTER(&HttpStateMachine::state_congestion_control_lookup, event);
...
void
HttpStateMachine::redirect_request(const char *redirect_url, const int redirect_len)
{
...
void
HttpStateMachine::set_http_schedule(Continuation *contp)
{
	HTTP_SM_SET_DEFAULT_HANDLER(&HttpStateMachine::get_http_schedule);
	schedule_cont = contp;
}

int
HttpStateMachine::get_http_schedule(int event, void * /* data ATS_UNUSED */)
{
	bool plugin_lock;
	Ptr <ProxyMutex> plugin_mutex;
...
bool
HttpStateMachine::set_server_session_private(bool private_session)
{
	if (server_session != NULL) {
...
inline bool
HttpStateMachine::is_private()
{
	bool result = false;

	if (server_session) {

		result = server_session->private_session;

	} else if (ua_session) {

		HttpServerSession * server_session = ua_session->get_server_session();
		if (server_session) {
			result = server_session->private_session;
		}
	}
