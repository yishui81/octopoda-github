                          ...
                          int raw_tag_field_len, const char *comma_sep_tag_list, int comma_sep_tag_list_len)
{
	...
	for (Str * tag = tag_list.head; tag; tag = tag->next) {
		...
		if (((int) (tag->len - n) == etag_length) &&
				(strncmp(etag_start, tag->str + n, etag_length) == 0)) {
			return true;
		}
	}
                        ...
                        int raw_tag_field_len, const char *comma_sep_tag_list, int comma_sep_tag_list_len)
{
	...
	for (Str * tag = tag_list.head; tag; tag = tag->next) {
		...
		if ((cur_tag_len == etag_length) &&
				(strncmp(cur_tag, etag_start, cur_tag_len) == 0)){
			return true;
		}
...
{
	while (*range1 && *range2 &&
			(ParseRules::ink_tolower(*range1) == ParseRules::ink_tolower(*range2))) {
		range1 += 1;
		range2 += 1;
	}
