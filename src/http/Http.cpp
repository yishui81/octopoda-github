...
void
http_hdr_adjust(HttpHeaderImpl */* hdrp ATS_UNUSED */, int32_t /* offset ATS_UNUSED */, int32_t /* length ATS_UNUSED */,
...
http_hdr_create(HdrHeap *heap, HTTPType polarity)
{
	...
	hh = (HttpHeaderImpl *) heap->allocate_obj(sizeof(HttpHeaderImpl), HDR_HEAP_OBJ_HTTP_HEADER);
...
void
http_hdr_init(HdrHeap *heap, HttpHeaderImpl *hh, HTTPType polarity)
{
...
void
http_hdr_copy_onto(HttpHeaderImpl *s_hh, HdrHeap *s_heap, HttpHeaderImpl *d_hh, HdrHeap *d_heap, bool inherit_strs)
{
	...
	memcpy(d_hh, s_hh, sizeof(HttpHeaderImpl));
...
HttpHeaderImpl *
http_hdr_clone(HttpHeaderImpl *s_hh, HdrHeap *s_heap, HdrHeap *d_heap)
{
	HttpHeaderImpl *d_hh;
...
int
http_hdr_print(HdrHeap *heap, HttpHeaderImpl *hdr, char *buf, int bufsize, int *bufindex, int *dumpoffset)
{
	#define TRY(x)  if (!x) return 0
	...
	if (hdr->m_polarity == HTTP_TYPE_REQUEST) {
		...
		if ((buf != NULL) && (*dumpoffset == 0)
				&& (bufsize - *bufindex >= hdr->u.req.m_len_method + 1)) {  // fastpath
		...
		if ((buf != NULL) && (*dumpoffset == 0)
				&& (bufsize - *bufindex >= 9 + 6 + 1)) {    // fastpath
...
{
	HttpHeaderImpl *obj = (HttpHeaderImpl *) raw;
...
int
http_hdr_length_get(HttpHeaderImpl *hdr)
{
	int length = 0;
...
void
http_hdr_type_set(HttpHeaderImpl *hh, HTTPType type)
{
	hh->m_polarity = type;
}
...
void
http_hdr_version_set(HttpHeaderImpl *hh, int32_t ver)
{
	hh->m_version = ver;
}
...
const char *
http_hdr_method_get(HttpHeaderImpl *hh, int *length)
{
	const char *str;
...
void
http_hdr_method_set(HdrHeap *heap, HttpHeaderImpl *hh, const char *method, int16_t method_wks_idx, int method_length,
                    bool must_copy)
{
	ink_assert(hh->m_polarity == HTTP_TYPE_REQUEST);
...
void
http_hdr_url_set(HdrHeap *heap, HttpHeaderImpl *hh, URLImpl *url)
{
	ink_assert(hh->m_polarity == HTTP_TYPE_REQUEST);
...
void
http_hdr_status_set(HttpHeaderImpl *hh, HTTPStatus status)
{
...
const char *
http_hdr_reason_get(HttpHeaderImpl *hh, int *length)
{
	ink_assert(hh->m_polarity == HTTP_TYPE_RESPONSE);
...
void
http_hdr_reason_set(HdrHeap *heap, HttpHeaderImpl *hh, const char *value, int length, bool must_copy)
{
	ink_assert(hh->m_polarity == HTTP_TYPE_RESPONSE);
...
MIMEParseResult
http_parser_parse_req(HTTPParser *parser, HdrHeap *heap, HttpHeaderImpl *hh, const char **start, const char *end,
...
MIMEParseResult
http_parser_parse_resp(HTTPParser *parser, HdrHeap *heap, HttpHeaderImpl *hh, const char **start, const char *end,
                       bool must_copy_strings, bool eof)
{
	if (parser->m_parsing_http) {
...
//		ink_assert((end - cur) < UINT16_MAX);
		...
		if (end - cur >= 16) {
			...
			if ((http_match != 0) ||
			  (!(is_digit(cur[5]) && is_digit(cur[7]) &&
				 is_digit(cur[9]) && is_digit(cur[10]) &&
			...
			reason_start = &(cur[13]);
			reason_end = end - 1;
			while ((reason_end > reason_start + 1) &&
					(ParseRules::is_space(reason_end[-1]))){
				--reason_end;
			}
	...
	parse_reason1:
		...
		reason_start = cur;
		reason_end = end - 1;
		while ((reason_end >= line_start) &&
				(ParseRules::is_cr(*reason_end) || ParseRules::is_lf(*reason_end))) {
			reason_end -= 1;
		}
		reason_end += 1;
		goto done;
